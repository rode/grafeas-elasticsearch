...
echo "start es"
docker-compose up --build -d elasticsearch grafeas-next
echo "dc up finished"
...
docker-compose stop grafeas-next

cp occurrences.json ../../mappings

FAIL_AFTER_STEP="ADD_WRITE_BLOCK" docker-compose up --build grafeas-next

echo "restarting grafeas"
