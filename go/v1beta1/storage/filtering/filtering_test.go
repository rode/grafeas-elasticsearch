...
var _ = Describe("Filter", func() {
	Describe("ParseExpression", func() {
		DescribeTable("filter cases", func(filter string, expected interface{}) {
			...
			// build.provenance.builtArtifacts.*.names == "foo"
			// ==
			// -> build.provenance.builtArtifacts
			//
			...
			//// build.provenance.builtArtifacts["names" == "abc"]
			//
			// build.provenance.builtArtifacts.filter(a, c.names === "")
			//Entry("foo", `a.b.filter(c, c.name == "foo")`, &Query{
			//	Nested: &Nested{
			//		Path: "a.b",
			//		Query: &Query{
			//			Term: &Term{
			...
			//	},
			//}),
			Entry("complex", `a.b.filter(c, (c.name == "foo") && (d == e))`, &Query{
				Nested: &Nested{
					Path: "a.b",
					Query: &Query{
						Bool: &Bool{
